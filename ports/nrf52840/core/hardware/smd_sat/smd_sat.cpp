...
bool SmdSat::parse_aplus_response(const uint8_t *rx_buffer, uint16_t rx_len, SpiAplusResponse *response) {
    ...
    // Check for BUSY pattern (0xBB in first bytes)
    if (rx_buffer[0] == SPI_PROTOCOL_APLUS_BUSY_PATTERN &&
        rx_buffer[1] == SPI_PROTOCOL_APLUS_BUSY_PATTERN) {
        DEBUG_TRACE("SmdSat::%s: BUSY pattern detected", __func__);
        response->status = SPI_APLUS_STATUS_BUSY;
        return false;
    }

    // Check for IDLE pattern (no valid response yet)
    if (rx_buffer[0] == SPI_PROTOCOL_APLUS_IDLE_PATTERN &&
        rx_buffer[1] == SPI_PROTOCOL_APLUS_IDLE_PATTERN &&
        rx_buffer[2] == SPI_PROTOCOL_APLUS_IDLE_PATTERN) {
    ...
    if (rx_len < 5) {
        DEBUG_WARN("SmdSat::%s: Response too short (%u bytes)", __func__, rx_len);
        return false;
    }

    // Check magic byte
    if (rx_buffer[0] != SPI_PROTOCOL_APLUS_MAGIC_RESPONSE) {
        DEBUG_WARN("SmdSat::%s: Invalid magic byte 0x%02X (expected 0x%02X)",
                   __func__, rx_buffer[0], SPI_PROTOCOL_APLUS_MAGIC_RESPONSE);
        response->status = SPI_APLUS_STATUS_FRAME_ERROR;
        return false;
    }

    // Parse header
    response->seq = rx_buffer[1];
    response->status = static_cast<SpiAplusStatus>(rx_buffer[2]);
    ...
    if (data_len > 0) {
        memcpy(response->data, &rx_buffer[4], data_len);
    }

    // Validate CRC-8 over MAGIC + SEQ + STATUS + LEN + DATA (includes MAGIC!)
    uint16_t crc_data_len = SPI_PROTOCOL_APLUS_HEADER_LEN + data_len;
    uint8_t received_crc = rx_buffer[crc_data_len];
    uint8_t calculated_crc = spi_crc8_ccitt(rx_buffer, crc_data_len);
...
bool SmdSat::send_command_aplus(uint8_t command, const uint8_t *tx_data, uint16_t tx_len,
    ...
    // Build the A+ frame (fixed 64 bytes)
    ...
    // Wait for SMD to process command
    ...
    // Build NOP frame (command 0x00)
    uint8_t saved_seq = m_sequence_number;
    m_sequence_number++;  // Increment for NOP
    build_aplus_frame(nop_frame, SMDSAT_CMD_NONE, nullptr, 0);

    memset(response_frame, 0, sizeof(response_frame));

    // Wait for processing
    nrf_delay_ms(SMDSAT_TIMING_STANDARD_MS);
    ...
    // Handle BUSY pattern with retries
    for (uint8_t busy_retry = 0; busy_retry < SMDSAT_SPI_BUSY_MAX_RETRIES; busy_retry++) {
        if (response_frame[0] == SPI_PROTOCOL_APLUS_BUSY_PATTERN &&
            response_frame[1] == SPI_PROTOCOL_APLUS_BUSY_PATTERN) {
            DEBUG_TRACE("SmdSat::%s: BUSY, retry %u/%u", __func__, busy_retry + 1, SMDSAT_SPI_BUSY_MAX_RETRIES);
            nrf_delay_ms(SMDSAT_SPI_BUSY_RETRY_DELAY_MS);

            // Send another NOP to poll for response
            m_sequence_number++;
            build_aplus_frame(nop_frame, SMDSAT_CMD_NONE, nullptr, 0);
            memset(response_frame, 0, sizeof(response_frame));
    ...
    if (response != nullptr) {
        // Restore sequence number for validation (response should match original command seq)
        uint8_t current_seq = m_sequence_number;
        m_sequence_number = saved_seq;

        bool parsed = parse_aplus_response(response_frame, SPI_PROTOCOL_APLUS_FRAME_SIZE, response);

        m_sequence_number = current_seq;
...
bool SmdSat::detect_protocol() {
    DEBUG_TRACE("SmdSat::%s: Detecting protocol mode", __func__);
    ...
    build_aplus_frame(tx_frame, SMDSAT_CMD_PING, nullptr, 0);
    memset(rx_frame, 0, sizeof(rx_frame));

    int ret = m_nrf_spim->transfer(tx_frame, rx_frame, SPI_PROTOCOL_APLUS_FRAME_SIZE);
    if (ret) {
        DEBUG_WARN("SmdSat::%s: A+ ping transfer failed, falling back to legacy", __func__);
        m_protocol_mode = SpiProtocolMode::LEGACY;
    ...
    // Send NOP to get response (pipelined)
    uint8_t nop_frame[SPI_PROTOCOL_APLUS_FRAME_SIZE];
    uint8_t response_frame[SPI_PROTOCOL_APLUS_FRAME_SIZE];
    ...
    // Check if response looks like A+ (starts with magic 0x55)
    if (response_frame[0] == SPI_PROTOCOL_APLUS_MAGIC_RESPONSE) {
        DEBUG_INFO("SmdSat::%s: Protocol A+ detected", __func__);
    ...
    // Check for BUSY/IDLE patterns (also indicates A+ mode)
    if (response_frame[0] == SPI_PROTOCOL_APLUS_BUSY_PATTERN ||
        response_frame[0] == SPI_PROTOCOL_APLUS_IDLE_PATTERN) {
        DEBUG_INFO("SmdSat::%s: Protocol A+ detected (pattern 0x%02X)", __func__, response_frame[0]);
        m_protocol_mode = SpiProtocolMode::APLUS;
        m_protocol_detected = true;
        m_sequence_number++;
        return true;
    ...
    // Legacy response
    DEBUG_INFO("SmdSat::%s: Legacy protocol detected (response[0]=0x%02X)", __func__, response_frame[0]);
    m_protocol_mode = SpiProtocolMode::LEGACY;
    m_protocol_detected = true;
    return false;
}

// Helper to check if we need longer delay for flash operations
...
bool SmdSat::send_command_auto(uint8_t command, const uint8_t *tx_data, uint16_t tx_len,
    ...
    // Auto-detect protocol on first use
    if (!m_protocol_detected) {
        detect_protocol();
    }

    if (m_protocol_mode == SpiProtocolMode::APLUS) {
        // Use Protocol A+ framing with retry logic
        SpiAplusResponse response;
        ...
        for (uint8_t retry = 0; retry < SMDSAT_SPI_MAX_RETRIES; retry++) {
            if (send_command_aplus(command, tx_data, tx_len, &response)) {
                ...
                if (rx_data != nullptr && rx_len > 0 && response.data_len > 0) {
                ...
                if (!SPI_APLUS_IS_RECOVERABLE(response.status)) {
                    DEBUG_WARN("SmdSat::%s: Non-recoverable error %u", __func__, static_cast<int>(response.status));
                    return false;
                }
            }

            DEBUG_TRACE("SmdSat::%s: Retry %u/%u", __func__, retry + 1, SMDSAT_SPI_MAX_RETRIES);
        ...
        try {
            if (tx_data == nullptr || tx_len == 0) {
                ...
                uint8_t *full_tx = static_cast<uint8_t*>(calloc(tx_len + 1, sizeof(uint8_t)));
                uint8_t *full_rx = static_cast<uint8_t*>(calloc(tx_len + 1, sizeof(uint8_t)));

                if (full_tx == nullptr || full_rx == nullptr) {
                    free(full_tx);
                    free(full_rx);
                    return false;
                }
                ...
                full_tx[0] = command;
                memcpy(full_tx + 1, tx_data, tx_len);
                send_command(full_tx, full_rx, tx_len + 1);

                if (rx_data != nullptr && rx_len > 0) {
                    uint16_t copy_len = (tx_len < rx_len) ? tx_len : rx_len;
                    memcpy(rx_data, full_rx + 1, copy_len);
                }
...
void SmdSat::read_radio_conf(SmdArgosModulation *modulation){
    ...
    *modulation = static_cast<SmdArgosModulation>(rx[9]);
    ...
    DEBUG_INFO("SmdSat::%s:Modulation: %u, Min Frequency: %u, Max Frequency: %u, RF Level %d", __func__,
...
void SmdSat::read_seckey(smd_uint8_array_t *seckey){
	...
	if(seckey->size != SMDSAT_CMD_READ_SECKEY_LEN){
		DEBUG_ERROR("SmdSat::%s:Size allocated to store seckey %u != from %u",
...
void SmdSat::read_tcxo_warmup(uint8_t *time_ms){
	...
	send_command(SMDSAT_CMD_READ_TCXO);
	nrf_delay_ms(SMDSAT_DELAY_CMD_MS);
	uint8_t tx[SMDSAT_CMD_READ_ID_LEN] = {0};
	uint8_t rx[SMDSAT_CMD_READ_ID_LEN] = {0};
	send_command(tx, rx, SMDSAT_CMD_READ_ID_LEN);
...
void SmdSat::write_tcxo_warmup(uint32_t time_ms){
	...
	send_command(tx, rx, SMDSAT_CMD_WRITE_TCXO_LEN);
}

void SmdSat::read_serial(smd_uint8_array_t *serial){
	DEBUG_TRACE("%s",__func__);
	if(serial->size != SMDSAT_CMD_READ_SERIAL_LEN){
		DEBUG_ERROR("SmdSat::%s:Size allocated to store serial %u != from %u",
...
bool SmdSat::smd_ping()
{
	...
	// If protocol not yet detected, try detection with A+ first
	if (!m_protocol_detected && m_nrf_spim != nullptr) {
		if (detect_protocol()) {
			// Protocol A+ detected and ping succeeded during detection
			DEBUG_INFO("SmdSat::%s: ACK from SMD received (A+ protocol)",__func__);
			return true;
		}
		// If detection failed or legacy detected, continue with legacy ping below
	}

	// Use Protocol A+ if detected
	if (m_protocol_detected && m_protocol_mode == SpiProtocolMode::APLUS) {
		for (uint8_t retry = 0; retry < SMDSAT_SPI_MAX_RETRIES; retry++) {
			SpiAplusResponse response;
			try {
				if (send_command_aplus(SMDSAT_CMD_PING, nullptr, 0, &response)) {
					if (response.status == SPI_APLUS_STATUS_OK) {
						DEBUG_INFO("SmdSat::%s: ACK from SMD received (A+)",__func__);
						return true;
					}
				}
			} catch (...) {
				DEBUG_WARN("SmdSat::%s: A+ Ping failed, retry %u/%u", __func__, retry + 1, SMDSAT_SPI_MAX_RETRIES);
			}
			nrf_delay_ms(SMDSAT_SPI_RETRY_DELAY_MS);
		}
	}

	// Legacy protocol ping
	for (uint8_t retry = 0; retry < SMDSAT_SPI_MAX_RETRIES; retry++) {
		try {
			...
			read_byte(&ping);
			if (ping == 1) {
				DEBUG_INFO("SmdSat::%s: ACK from SMD received (legacy)",__func__);
				return true;
			} else {
...
bool SmdSat::is_tx_finished() {
	...
	return (kmac_status == MAC_TX_DONE);
}
...
SmdSat::SmdSat(unsigned int idle_shutdown_ms) {
	...
	m_protocol_mode = SpiProtocolMode::AUTO_DETECT;
	m_sequence_number = 0;
	m_protocol_detected = false;
	// DFU initialization
	m_dfu_mode = false;
...
void SmdSat::state_starting()
{
	m_is_first_tx = true;
	is_kmac_profil_loaded = false;
	// Reset Protocol A+ state for fresh detection after power cycle
	m_protocol_detected = false;
	m_sequence_number = 0;
...
void SmdSat::initiate_tx() {
	...
	uint8_t *tx_msg = static_cast<uint8_t*>(calloc(size + 1, sizeof(uint8_t)));
	uint8_t *rx_msg = static_cast<uint8_t*>(calloc(size + 1, sizeof(uint8_t)));

	if (tx_msg == NULL || rx_msg == NULL) {
		DEBUG_ERROR("SmdSat::%s: Memory allocation failed", __func__);
		free(tx_msg);
		free(rx_msg);
		return;
	}
...
SmdDfuResponse SmdSat::dfu_send_command(uint8_t cmd, const uint8_t *data, uint16_t data_len,
	...
	DEBUG_TRACE("SmdSat::%s: cmd=0x%02X, data_len=%u", __func__, cmd, data_len);

	SpiAplusResponse response;

	if (!send_command_aplus(cmd, data, data_len, &response)) {
		DEBUG_ERROR("SmdSat::%s: Command 0x%02X failed", __func__, cmd);
	...
	// Map SPI A+ status to DFU response
	if (response.status == SPI_APLUS_STATUS_OK) {
		// Check if first data byte contains DFU-specific status
		if (response.data_len > 0) {
			return static_cast<SmdDfuResponse>(response.data[0]);
		}
		return DFU_RSP_OK;
	}
...
SmdDfuResponse SmdSat::dfu_get_info(SmdDfuInfo *info) {
	...
	if (result == DFU_RSP_OK && response_len >= 14) {
		info->version_major = response_data[0];
		info->version_minor = response_data[1];
		info->app_start_addr = (response_data[2] << 24) | (response_data[3] << 16) |
		                       (response_data[4] << 8) | response_data[5];
		info->app_max_size = (response_data[6] << 24) | (response_data[7] << 16) |
		                     (response_data[8] << 8) | response_data[9];
		info->flash_page_size = (response_data[10] << 24) | (response_data[11] << 16) |
		                        (response_data[12] << 8) | response_data[13];

		DEBUG_INFO("SmdSat::%s: Bootloader v%u.%u, app_start=0x%08X, max_size=%u, page_size=%u",
		           __func__, info->version_major, info->version_minor,
...
SmdDfuResponse SmdSat::dfu_erase() {
	...
	if (result == DFU_RSP_OK) {
		...
		nrf_delay_ms(SMDSAT_DFU_ERASE_TIMEOUT_MS);
...
bool SmdSat::dfu_enter() {
	...
	// Reset protocol state for fresh detection
	m_protocol_detected = false;
	m_sequence_number = 0;

	// Wait for SMD to be ready
	nrf_delay_ms(SMDSAT_DELAY_POWER_ON_MS / 2);

	// Send DFU_ENTER command (0x3F)
	DEBUG_INFO("SmdSat::%s: Sending DFU_ENTER command", __func__);

	for (uint8_t retry = 0; retry < SMDSAT_DFU_MAX_RETRIES; retry++) {
		try {
			send_command(SMDSAT_CMD_DFU_ENTER);
			nrf_delay_ms(SMDSAT_DELAY_CMD_MS);
			...
			if (ack == 0x01) {
				DEBUG_INFO("SmdSat::%s: DFU_ENTER ACK received, waiting for bootloader", __func__);

				// Wait for SMD to reset into bootloader
				nrf_delay_ms(SMDSAT_DFU_RESET_DELAY_MS * 2);

				// Reset protocol state after SMD reset
				m_protocol_detected = false;
				m_sequence_number = 0;

				// Try to ping the bootloader
				for (uint8_t ping_retry = 0; ping_retry < 5; ping_retry++) {
					nrf_delay_ms(200);
					SmdDfuResponse ping_result = dfu_ping();
					if (ping_result == DFU_RSP_OK) {
						m_dfu_mode = true;
						DEBUG_INFO("SmdSat::%s: DFU mode entered successfully", __func__);
						return true;
					}
				}
			...
			DEBUG_WARN("SmdSat::%s: DFU_ENTER no ACK (got 0x%02X), retry %u/%u",
			           __func__, ack, retry + 1, SMDSAT_DFU_MAX_RETRIES);

		} catch (...) {
			DEBUG_WARN("SmdSat::%s: DFU_ENTER failed, retry %u/%u",
			           __func__, retry + 1, SMDSAT_DFU_MAX_RETRIES);
		}
		nrf_delay_ms(SMDSAT_SPI_RETRY_DELAY_MS);
	}
...
std::string SmdSat::get_firmware_version() {
	...
	try {
		// Send read firmware command
		send_command(SMDSAT_CMD_READ_FIRMWARE);
		nrf_delay_ms(SMDSAT_DELAY_CMD_MS);

		// Read firmware version string (128 bytes max)
		uint8_t tx[SMDSAT_CMD_READ_FIRMWARE_LEN] = {0};
		uint8_t rx[SMDSAT_CMD_READ_FIRMWARE_LEN] = {0};
		send_command(tx, rx, (uint16_t)SMDSAT_CMD_READ_FIRMWARE_LEN);
		...
		size_t len = 0;
		while (len < SMDSAT_CMD_READ_FIRMWARE_LEN && rx[len] != 0) {
			len++;
		}
	...
	// Power off SMD if we powered it on
	if (was_stopped) {
		shutdown();
	}
