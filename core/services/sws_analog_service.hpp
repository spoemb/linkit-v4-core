...
public:
    ...
    struct Status {
        ...
        // Detection diagnostics (new - for IHM)
        uint8_t  last_detect_method;  // DetectMethod enum of last state change
        uint8_t  last_drop_percent;   // Drop % that triggered last detection (0-100)
        uint16_t last_drop_absolute;  // Absolute ADC drop of last detection
        uint8_t  trend_count;         // Current decreasing trend count
        uint8_t  consecutive_samples; // Current consecutive confirmation count
        uint16_t contrast_ratio_x10;  // water/air ratio * 10 (e.g. 87 = 8.7x)
        uint16_t midpoint;            // Current midpoint (air+water)/2
    };
